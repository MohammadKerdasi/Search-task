...
function getSearchMethod(id) {
  ...
  if(id == 'search'){
    searchBtn = 'name'
    search.placeholder = 'Search row'
  }
  search.focus()
 
}

function searchRow(value) {
  if (searchBtn === 'First') {
    let table = '';
    for (let elementIndex = 0; elementIndex < data.length; elementIndex++) {
      if (data[elementIndex] && data[elementIndex].name && data[elementIndex].name.includes(value)) {
        table += `
          <tr>
            <th scope="row">${elementIndex}</th>
            <td>${data[elementIndex].name}</td>
            <td>${data[elementIndex].last}</td>
            <td>${data[elementIndex].handle}</td>
            <td>      
              <button data-index=${elementIndex} onclick="deleteRow(event)" class="btn btn-danger delete-button-${elementIndex}">Delete row</button>
              <button data-index=${elementIndex} onclick="updateRow(event)" class="btn btn-warning delete-button-${elementIndex}">Update</button>
            </td>
